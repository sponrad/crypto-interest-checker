...
This repo includes a multi-stage `Dockerfile` that runs `npm run build` (Vite) and serves `dist/` with nginx (SPA routing enabled).
...
Click **Deploy**. First build takes a few minutes (npm install + vite build). When it’s green, open `https://crypto.devlabtech.com`.
...
The Dockerfile approach is more predictable for production builds.
